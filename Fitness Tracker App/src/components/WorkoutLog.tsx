...
const WorkoutLog: React.FC = () => {
  ...
  // Calculate calories burned whenever activity type or duration changes
  useEffect(() => {
    if (workoutData.activityType && workoutData.duration) {
      const caloriesBurned = Math.round(
        CALORIE_BURN_RATES[workoutData.activityType] * workoutData.duration
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      ...
      Object.values(workoutData).some((value) => !value)
    ) {
      alert('Please fill all fields correctly');
      return;
    }

    // Here you could implement an API call to save workout data.
    console.log('Workout data to be saved:', workoutData);

    // Add the new workout to the recent workouts list
    const newWorkout: WorkoutData = { ...workoutData, id: Date.now() };
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white p-6 rounded-lg shadow mb-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="activityType" className="block mb-1 font-medium">
              Activity Type
            </label>
            <select
              id="activityType"
          ...
          <div>
            <label htmlFor="caloriesBurned" className="block mb-1 font-medium">
              Calories Burned
            </label>
            <input
              type="number"
            ...
            {recentWorkouts.map((workout) => (
              <li key={workout.id} className="flex items-center justify-between bg-gray-50 p-3 rounded">
                <div className="flex items-center">
                  ...
                  <span className="font-medium">{workout.activityType}</span>
                </div>
